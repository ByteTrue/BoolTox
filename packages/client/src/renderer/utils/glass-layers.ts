...
export function getGlassStyle(
  ...
  const { withBorderGlow = true, withInnerShadow = true, customBorderColor } = options || {};

  // 选择正确的层级映射
  const layers = theme === 'dark' ? GLASS_LAYERS_DARK : GLASS_LAYERS;
  const borderColor =
    customBorderColor || (theme === 'dark' ? GLASS_BORDERS.DARK : GLASS_BORDERS.LIGHT);
  ...
  if (withBorderGlow) {
    // 顶部微光边框 - 浅色用深色，深色用白色
    shadows.push(
      theme === 'dark' ? '0 0.5px 0 0 rgba(255, 255, 255, 0.1)' : '0 0.5px 0 0 rgba(0, 0, 0, 0.05)' // 浅色模式用深色边框高光
    );
  }
  if (withInnerShadow) {
...
export function getGlassShadow(theme: 'dark' | 'light'): string {
  return theme === 'dark' ? 'shadow-unified-lg-dark' : 'shadow-unified-lg';
}
...
export function getGlassActiveStyle(theme: 'dark' | 'light' = 'light'): CSSProperties {
  return {
    ...
    borderColor: theme === 'dark' ? 'rgba(101, 187, 233, 0.4)' : 'rgba(81, 169, 213, 0.5)',
    backdropFilter: `blur(${GLASS_BLUR}px) saturate(${GLASS_SATURATE}%)`,
    WebkitBackdropFilter: `blur(${GLASS_BLUR}px) saturate(${GLASS_SATURATE}%)`,
    boxShadow:
      theme === 'dark'
