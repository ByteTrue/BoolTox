...
**为工具开发者提供类型安全的运行时 API 封装**：
- **前端 API**：工具 UI 与 BoolTox Client 通信（`api.ts`）
- **后端 API**：工具后端服务调用（`backend.ts`）
- **React Hooks**：简化工具开发的 Hooks（`hooks.ts`）
- **类型定义**：重导出 `@booltox/shared` 的类型

**核心理念**：
- **运行时无依赖**：工具打包后不携带 SDK 代码（BoolTox 提供运行时）
...
**A**：不可以，Hooks 依赖 React。非 React 工具应直接使用 `api.ts` 和 `backend.ts`。
...
- ⚠️ 待补充：实际工具开发示例（参考 `booltox-plugins` 仓库）
